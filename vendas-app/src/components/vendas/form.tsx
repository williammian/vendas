...
function formatarMoneySemPrefixo(valor: number) {
    return formatadorMoney.format(valor).replace(/^R\$\s?/, '');
}
...
export const VendasForm: React.FC<VendasFormProps> = ({
    ...
    const handleCodigoProdutoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
        if(codigoProduto){
    ...
    const handleProdutoAutocomplete = async (e: AutoCompleteCompleteMethodParams) => {
        ...
        const produtosEncontrados = listaProdutos.filter( (produto: Produto) => {
            return produto.nome?.toUpperCase().includes(e.query.toUpperCase()) ?? false;
        })
    ...
    const handleProdutoChange = (e: AutoCompleteChangeParams) => {
        ...
        setCodigoProduto(produtoSelecionado.id ?? '')
    }
    ...
    const totalVenda = () => {
        const totais: number[] = formik.values.itens?.map(iv => iv.quantidade * (iv.produto.preco ?? 0)) ?? [];
        if(totais.length){
                ...
                <div className="p-grid">
                    ...
                    <div className="p-col-2">
                        <span className="p-float-label">
                            <InputText id="qtdProduto" 
                                       ...
                                       onChange={e => setQuantidadeProduto(e.target.value ? parseInt(e.target.value) : 0)}
                                       />
                    ...
                    <div className="p-col-12">
                        <DataTable value={formik.values.itens} emptyMessage="Nenhum produto adicionado.">
                            ...
                            <Column header="Preço Unitário" body={(iv: ItemVenda) => {
                                const precoFormatado = formatarMoneySemPrefixo(iv.produto.preco ?? 0)
                                return (
                                    <div>
                                        { precoFormatado }
                                    </div>
                            ...
                            <Column header="Total" body={ (iv: ItemVenda) => {
                                const total = (iv.produto.preco ?? 0) * iv.quantidade
